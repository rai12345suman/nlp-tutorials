...
def print_tags(document):
    tagdict = findtags('NN', nltk.corpus.brown.tagged_words(categories='news'))
    for tag in sorted(tagdict):
        print(tag, tagdict[tag])

def find_inaccuracies(errors, classifier, test_set, word_features):
    """Using the test set, we can generate a list of the errors that the classifier makes when predicting"""
    for (line, tag) in test_set:
        guess = classifier.classify(find_features(line, word_features))
        if guess != tag:
            errors.append((tag, guess, line))
            print('correct={:<8} guess={:<8s} line={:<30}'.format(tag, guess,
                line))
...
def main():
    ...
    try:
        ...
        print(traceback.format_exc(e))
        return
    find_inaccuracies(errors, classifier, test_set, word_features)
